...
const { clienteleRedis } = require('../clienteleRedis');
...
const processProcessorRow = async (row, currentNumber, totalFetched) => {
    try {
        ...
        let clientele;
        const cachedNetworkType = await clienteleRedis.get(cellno);
        if (cachedNetworkType) {
            console.log(`Row ${currentNumber}/${totalFetched} (${row.id}) clientele cache hit for ${cellno}: ${cachedNetworkType}`);
            clientele = { carrier: 'zong', success: true, record: { network_type: cachedNetworkType } };
        } else {
            clientele = await checkZongNum({ cellno, subDomain: row.sub_domain });
        }
